...
- [x] **Släktträd-vy på /persons/{n}.** Familj-sektionen visar nu föräldrar/
  ...
  `region_id`).
- [x] **Smartare familjelänkar (partner/barn-symmetri).** Efter att en relation
  lagts till på /persons/{n} föreslås kompletterande förälder-länkar i ett modal
...
- [x] **Mini-släktträd på /persons/{n}.** Kompakt family-chart centrerat på personen
  (egna små kort via `setCardInnerHtmlCreator`/`setCardDim`) + delete-lista, ritas
  om dynamiskt vid relationsändring (`refreshFamily()`, ingen reload). Länk till
  fulla `/persons/tree?start={n}`.
- [ ] **Live-redigera släktträdet i family-chart edit-läge.** family-chart har
  inbyggd redigering (lägg till/koppla släktingar direkt i trädet via klick). Utvärdera
  att aktivera den i `/persons/tree` (och ev. mini-trädet) och spegla ändringar till
  `PersonLink` via relations-endpoints - så man bygger trädet visuellt. (Dynamisk
  omritning vid tillägg via fältet finns redan på /persons/{n}.) Docs:
